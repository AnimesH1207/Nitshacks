...
contract CredentialsPassport {
    ...
    struct Credential {
        uint256 id;
        address issuer;           // Address of the institution issuing the credential
        address student;           // Address of the student receiving the credential
        string credentialType;     // e.g., "B.Tech Degree", "Course Certificate"
        string institutionName;    // Name of the issuing institution
        uint256 issueDate;         // Timestamp when issued
        uint256 expiryDate;        // 0 means permanent, otherwise expiry timestamp
        bool revoked;              // Can be revoked by issuer
        string metadataURI;        // IPFS or other URI for additional data
        bytes32 commitment;       // ZKG commitment hash for selective disclosure
    }
    ...
    // Array of all registered issuers (for the registry)
    ...
    // Owner of the contract (can register issuers)
    ...
    /**
     * @dev Register a new issuer (only owner can do this)
     */
    ...
    function issueCredential(
        ...
        // Commitment can be zero for non-ZKG credentials
        if (commitment != bytes32(0)) {
            require(commitmentToCredential[commitment] == 0, "Commitment already exists");
        }
    ...
    /**
     * @dev Revoke a credential (only the issuer can revoke)
     */
    ...
    function verifyZKProof(
        ...
        // Verify proof hash (simplified ZK proof - in production, use proper ZK circuit like Circom)
        bytes32 computedProof = keccak256(abi.encodePacked(commitment, nonce, cred.issuer, cred.student));
        if (computedProof != proofHash) {
            return (false, 0);
        }
        
        emit ZKProofVerified(credId, commitment, true);
        return (true, credId);
    }
